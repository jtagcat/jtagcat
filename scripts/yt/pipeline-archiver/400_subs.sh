...
for categorypath in "$DLBASE/"*; do if [ -s "$categorypath/_indexes/known_urls.txt" ]; then
		...
		--write-subs --sub-langs 'en.*,et.*' --write-auto-subs --extractor-args youtube:skip=translated_subs `# for no live chats (if currnetly live is filtered seperately): --sub-langs all,-live_chat` \
